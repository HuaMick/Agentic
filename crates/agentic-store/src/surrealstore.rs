...
impl Store for SurrealStore {
    ...
    fn snapshot_for_story(&self, story_id: i64) -> Result<crate::StoreSnapshot, StoreError> {
        // For SurrealStore, the implementation mirrors MemStore's approach:
        // compute the closure via story YAML, then collect matching uat_signings.
        // SurrealStore mirrors the same tests as MemStore, so the closure logic
        // must be identical.
        ...
        for row in all_signings {
            if let Some(row_story_id) = row.get("story_id").and_then(|v| v.as_i64()) {
                // Include if it's an ancestor, exclude if it's the subject story.
        ...
        Ok(crate::StoreSnapshot {
            schema_version: 1,
            signings,
        })
    }

    fn restore(&self, snapshot: &crate::StoreSnapshot) -> Result<(), StoreError> {
...
impl SurrealStore {
    /// Compute the transitive closure of ancestor story IDs (mirrors MemStore).
